...
describe('OnDeviceComponent', function () {
	...
	describe('setValueAtKeyPath', function () {
		async function setAndVerifyValue(base: KeyPathBaseTypes, keyPath: string, value: any, expectedStartingValue: any) {
			const {value: startingValue} = await odc.getValueAtKeyPath(base, keyPath);
			expect(startingValue).to.equal(expectedStartingValue, `${base}.${keyPath} did not match expected value before set`);
			const result = await odc.setValueAtKeyPath(base, keyPath, value);
			expect(result.success).to.be.true;
			const {value: endingValue} = await odc.getValueAtKeyPath(base, keyPath);
			expect(endingValue).to.equal(value, `${base}.${keyPath} did not match expected value after set`);
		}
